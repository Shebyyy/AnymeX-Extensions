...
const PHASES: Phase[] = [
  {
    ...
    steps: [
      ...
      },
      {
        ...
        path: [{ label: 'Downloads' }, { label: 'Allow Notifications' }, { label: 'Allow Background Run' }],
        instruction: 'Enable both "Allow Notifications" and "Allow Background Run" so downloads continue and you get progress updates.',
        tip: 'Without Background Run, downloads pause when you switch apps.',
      },
      {
        title: 'Set Unrestricted Background Usage',
        icon: <Zap className="w-4 h-4" />,
        path: [{ label: 'Downloads' }, { label: 'Set' }, { label: 'Allow Background Usage' }, { label: 'Unrestricted' }, { label: 'Go Back' }],
        instruction: 'Tap "Set" next to background usage → Allow Background Usage → select "Unrestricted" → go back.',
        tip: 'Unrestricted = full speed downloads, no battery optimization killing them.',
      },
    ],
  },
  {
