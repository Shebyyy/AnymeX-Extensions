...
function buildSearchString(params: {
  ...
  for (const p of params.platform) sp.append('platform', p)
  for (const t of params.type) sp.append('type', t)
  for (const l of params.lang) sp.append('lang', l)
...
function MultiSelectDropdown({
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 z-50 bg-[#16161e] border border-white/10 rounded-lg shadow-2xl p-1 max-h-72 overflow-y-auto w-56">
          {options.map(opt => (
            <button
              key={opt.key}
...
export default function ExtensionsApp({ view }: { view: ViewMode }) {
  ...
  // Initialize from localStorage (cardLayout only)
  useEffect(() => {
    try {
  ...
  // Read URL params on mount and when searchParams changes
  useEffect(() => {
  ...
  // URL sync: update URL when filters change (using replaceState for no scroll jump)
  const updateUrl = useCallback((params: {
    ...
    const searchStr = buildSearchString(params)
    const newUrl = `${pathname}${searchStr}`
  ...
  const togglePlatform = useCallback((p: string) => {
    setPlatformFilter(prev => {
      ...
      // Defer URL update
      setTimeout(() => {
        updateUrl({ platform: next, type: typeFilter, lang: langFilter, search, nsfw: showNsfw })
      }, 0)
      return next
    })
  }, [typeFilter, langFilter, search, showNsfw, updateUrl])

  const toggleType = useCallback((t: string) => {
    setTypeFilter(prev => {
      const next = prev.includes(t) ? prev.filter(x => x !== t) : [...prev, t]
      setTimeout(() => {
        updateUrl({ platform: platformFilter, type: next, lang: langFilter, search, nsfw: showNsfw })
      }, 0)
      return next
    })
  }, [platformFilter, langFilter, search, showNsfw, updateUrl])

  const toggleLang = useCallback((l: string) => {
    setLangFilter(prev => {
      const next = prev.includes(l) ? prev.filter(x => x !== l) : [...prev, l]
      setTimeout(() => {
        updateUrl({ platform: platformFilter, type: typeFilter, lang: next, search, nsfw: showNsfw })
      }, 0)
      return next
    })
  }, [platformFilter, typeFilter, search, showNsfw, updateUrl])

  const handleSearchChange = useCallback((value: string) => {
    setSearch(value)
    updateUrl({ platform: platformFilter, type: typeFilter, lang: langFilter, search: value, nsfw: showNsfw })
  }, [platformFilter, typeFilter, langFilter, showNsfw, updateUrl])

  const handleNsfwToggle = useCallback(() => {
    const next = !showNsfw
    setShowNsfw(next)
    updateUrl({ platform: platformFilter, type: typeFilter, lang: langFilter, search, nsfw: next })
  }, [platformFilter, typeFilter, langFilter, search, showNsfw, updateUrl])
        ...
        <div className="px-4 sm:px-6 py-3 sm:py-4 max-w-6xl mx-auto w-full">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex bg-white/5 rounded-lg border border-white/10 p-0.5">
                <Link
                  href={`/extensions${buildSearchString({ platform: platformFilter, type: typeFilter, lang: langFilter, search, nsfw: showNsfw })}`}
                ...
                <Link
                  href={`/repos${buildSearchString({ platform: platformFilter, type: typeFilter, lang: langFilter, search, nsfw: showNsfw })}`}
                  className={`px-2 sm:px-3 py-1 rounded-md text-[11px] sm:text-xs font-medium transition-all ${
